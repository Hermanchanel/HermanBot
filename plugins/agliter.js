...
let handler = async(m, { conn, text, args, usedPrefix }) => {
  ...
  await m.reply('Sedang membuat...')
      axios.get(`http://zekais-api.herokuapp.com/glitter?text=${text}`).then((res) => {
             ranp = getRandom('.gif')
             rano = getRandom('.webp')
             exec(`wget ${res.data.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                ...
                conn.sendFile(m.chat, buffer, 'gliter.webp', '', m, false, { asSticker: true })
                fs.unlinkSync(rano)
      })
   })
}
handler.help = ['aglitertext']
handler.tags = ['sticker']
handler.command = /^agliter(text)$/i
handler.owner = false
handler.mods = false
