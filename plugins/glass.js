...
`.trim()
 ...
 try {
  await m.reply('Sedang membuat...')
	...
	let buff = `https://some-random-api.ml/canvas/glass?avatar=${teks}`
          let voss = await fetch(buff)
	  let vuss = await ftype.fromStream(voss.body)
          if (vuss !== undefined) {

	 conn.sendFile(m.chat, await getBuffer(buff), 'glass.jpg', caption,  m)

     }	} else {
    m.reply(`Kirim perintah ${usedPrefix}glass dengan caption digambar atau reply gambar yang tersedia`)
     }
 } catch (e) {
