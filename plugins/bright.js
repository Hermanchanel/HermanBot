...
gay = `sayang`
  ...
  let buffer = await (await fetch('https://some-random-api.ml/canvas/brightness?avatar=' + img + '&apa=nani')).buffer() // Masukan ApiKeymu disini..
  let res = await fetch('https://raw.githubusercontent.com/herokuapp-com/kuhong-api/main/info/credit.json')
  let json = await res.json()

 await conn.sendFile(m.chat, buffer, 'gambar.jpg', 'Aduh Silauuu', m)
...
handler.help = ['bright (reply)']
handler.tags = ['editor']
handler.command = /^bright$/i
handler.owner = false
handler.mods = false
